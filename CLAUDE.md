...
- **GPU provider work, done in its own project (2026-09-08 afternoon):** `~/Work/onnxruntime-ggml`, branch `aarch64-conv2d`, a fork of thewh1teagle/onnxruntime-ggml with four commits: 2-D Conv, pools, BatchNormalization, PRelu; ReduceL2; the Linux arm64 bundle built without SVE; partial claims as connected groups. All three models now run whole on the M1 GPU through it with outputs matching onnxruntime CPU at cosine 1.0000: CLIP 43 ms (CPU 49), w600k_r50 74 ms with `conv2d_im2col=1` (CPU 89), det_10g 128 ms (CPU 151), at 12-15 W total against 26-27 W on the CPU. The `compute` setting (auto/cpu/gpu, ced6f8b) wires it in: `auto` self-tests a small convolution on the GPU and falls back to CPU with the reason in `status`. The index worker was switched to the GPU at 15:26 from image 12,815: 0.42 s per image (CPU path 0.50), whole machine 13.1 W (CPU path about 27 W) with the CPU 93% idle, worker RSS 1.7 GB, no errors. That worker was OOM-killed at 16:12 (2.6 GB peak) when a `photos search` opened a second GPU process beside it; queries and `status` now stay on the CPU (d6957aa, 79a2b21), one GPU process at a time. With the two unused landmark models dropped (81d52fe) the restarted worker does 0.29 s per image at 13.5 W, RSS 1.5 GB. The hourly sync timer is enabled (the unit had a stale `--albums` flag, fixed in 6493da9); its first run at 16:21 found nothing new. The provider is installed in `.venv` as an editable install of `~/Work/onnxruntime-ggml/python` with the built `.so` copied into its `lib/`; other users get it with `pip install onnxruntime-ggml` once the fixed wheel is published. Published 2026-09-08 on the user's instruction: this repository at https://github.com/Sonstebo/icloud-photos-cli (public, `origin`), the fork at https://github.com/Sonstebo/onnxruntime-ggml, and upstream pull requests thewh1teagle/onnxruntime-ggml #4 (arm64 bundle without SVE), #5 (partial claims), #6 (2-D vision ops). `gh` works again: the git credential helper now goes through the mise shim instead of a versioned path. The provider library for local use is `~/Work/onnxruntime-ggml/target/release/libonnxruntime_ggml.so` with the python package under `python/`.
- **Memory (2026-09-08).** 7.5 GB RAM, no swap, and `/tmp` is a 3.7 GB tmpfs. The first pass-1 run died at 3,990 images: the index loop kept every chunk's decoded thumbnails (fixed, 54c3515), and a parallel ggml build plus scratch files in `/tmp` pushed the machine into the OOM killer, which took the terminal's whole cgroup with it. Background workers now run as transient systemd user units (`icloud-photos-index-*`, `icloud-photos-sync-*`); the index was restarted the same day and continues from 3,990. It stopped again at 12,165 (15:12) on a connection reset inside a record lookup; lookups and downloads now retry with backoff (738492c) and it was restarted from there.

- **GUI (decided 2026-09-08 evening):** the user chose [lap](https://github.com/julyx10/lap) (Tauri + Rust + Vue, GPL-3.0, local CLIP ViT-B/32 and InsightFace) as the photo GUI, over my recommendation of a thin web viewer served by the CLI. Survey: Immich, PhotoPrism, LibrePhotos, Ente, HomeGallery, digiKam all own their index and files, none browses an external catalogue. lap is cloned at `~/Work/lap` with its own local brief (findings and plan); its CLIP is the same weights as ours, so embeddings can be shared; faces differ. Built and running the same evening (index worker stopped for the build; fat LTO off, see the lap brief). `photos lap-export` (d1fc7fa) writes the catalogue into lap's library: symlink album under the cache, rows with dates/GPS/favourites, our thumbnails, CLIP vectors, people, faces and collections; 500 assets in 1.2 s, tested against lap's real schema. The fetch-on-open hook is written in the lap fork (https://github.com/Sonstebo/lap, branch `fetch-on-open`: `albums.fetch_command`, migration 17, run from `preview://` when the file is missing) and lap runs that build here; the export sets the command to `photos lap-fetch {path}` (df59282). Pass 1 of the index finished 2026-09-08 17:39: 30,009 of 30,165 images, 37,504 faces, 10,103 named; The last 156 had no thumbnail rendition in iCloud (94 HEIC with medium only, 62 Fujifilm RAF with a 20 MB original only); the index now falls back to medium or a small non-RAW original and records the rest as source `none` (e967275). Pass 1 complete at 17:57: 30,165 of 30,165 (30,009 from thumbnails, 93 medium, 1 original, 62 nothing usable), 37,646 faces, 10,152 named. lap re-exported: 33,066 entries, 30,036 embeddings, 37,625 faces of 42 people. The cache budget (2 GB) now evicts thumbnails as mediums arrive; lap keeps its own copies. lap hung and could not close on first real use: the export's 200 px thumbnails made lap regenerate all of them (its default is 512) and orphan rows piled up (fixed, 7e98a50, re-exported), and upstream lap panics on exit without an analytics key (fixed in the fork). Dev-venv rule: keep `pip install -e .`; a plain `pip install .` there silently runs a stale copy. Later the same evening: thumbnails carry the linked file's mtime (lap's staleness rule), entries link the medium preview only so the viewer triggers the hook, file-less entries get an error thumbnail row (no ffmpeg storms), `--fetch-thumbs` downloads missing thumbnails including movies, cache budget set to 4 GB (thumbnails alone are ~2.2 GB). `lap-export --fetch-thumbs` downloaded the missing thumbnails: 32,908 entries have one, 68 do not. The GUI is installed as **Photos** (`~/Work/lap`, branch `photos-branding`, `scripts/install-linux.sh`) with a desktop entry, so it is searchable in the launcher; originals are never prefetched, `photos lap-fetch` gets a medium preview on click, and `original --pin` is how a user keeps one. A design audit on 2026-09-09 closed the paths that could lose or fail to show a photo: managed albums are skipped by the album scan, the startup folder sync and the manual folder sync; the fetch-on-open hook covers previews, the video server and (through `ensure_file_present` plus a retry) ordinary images; entries are named after the rendition they stand for; `photos refresh` keeps everything current hourly. The audit table is in the lap brief. Next: the assistant panel, pass 2 on medium previews, watch upstream (onnxruntime-ggml PRs #4-#6, lap issue #295).

- **The light table (2026-09-09):** many photos into one picture, the level above single-photo editing. `photos select` is a funnel, not a ranking: filters, meaning (CLIP nearest neighbours across the whole library, then filtered), near-duplicates collapsed (alike *and* within ninety seconds, keeper decided by focus unless you marked one yourself), a quality floor, then maximal marginal relevance with quotas for spreading over days and for making sure everyone named appears. Every stage reports what it removed, so an empty result explains itself. `photos compose` lays a set out in six templates (justified, grid, hero, filmstrip, spread, scatter) at six page shapes, with face-safe cropping built on the 37,646 face boxes already in the catalogue: no crop may cut a face, and when the stored boxes cannot be trusted the faces are dropped and the crop centred. Drafts use cached thumbnails and need no network; `--originals` fetches full files, refuses to start if they would not fit the cache budget, and composes at 300 dpi under an ImageMagick memory limit. `catalog._filters` was extracted so `count_assets` and `search` can never disagree, and `search` gained `people` (any of several). Measured: 33,161 assets to 8 photos in 1.4 s; a nine-photo draft in 0.2 s; an A4 page at 300 dpi from three fetched originals in 8.7 s. The GUI side is `t_table.rs` plus `LightTableDialog.vue` in the lap fork, reached from the file menu with several photos selected or from a collection's menu; the draft *is* the renderer, so the preview cannot drift from the print. Five bugs were found by looking at the rendered pages rather than the code (grid left a third of the page blank, the filmstrip floated in white, a leaked `-gravity south` shifted every tile, a sine hash piled the scattered prints into one corner) and two by the tests (a favourite could lose to a crisper unmarked photo; a lone scattered print was pushed off the page by its own tilt margin). 104 tests pass. Design published at https://claude.ai/code/artifact/2268465c-51fe-4c0d-a85c-07ce62871182. All five phases of that design are now built. `photos book` completes it: a book is an ordered list of pages, and a page is a recipe rather than a picture, so nothing is drawn until export and reordering always shows up in the result. Every page carries the book's shape, because a PDF gives its pages one size; that is also why a caption is written into a strip the layout reserves inside the page rather than spliced on afterwards, which had made captioned pages taller and distorted them when bound. Rendered pages carry no creation timestamp, so the same recipe gives the same file. In the app, the light table can ask for a set (candidates you keep or drop, never added silently) and keep the page it is showing into a book. Measured: a three-page A4 landscape book at 300 dpi in 22 s, the PDF measuring 842x595 points, which is A4. 131 tests pass.
